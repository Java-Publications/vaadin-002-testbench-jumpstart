...
//    public static final Supplier<String> sessionID = memoize(() -> session.get().getId());
    ...
    protected void postConstruct() {
        ...
        vaadinService.get().addSessionDestroyListener((SessionDestroyListener) event -> messageObservable.unregister(key));
        ...
        messageObservable
            .register(key,
                      (value) -> {
                          ...
                          Case
                              .match(
                                  Case.matchCase(() -> Result.success(" NO OP for " + value.toString())),
                                  Case.matchCase(key::isEmpty, () -> Result.failure("epmty event value")),
                                  Case.matchCase(() -> !key.equals(messageKey), () -> Result.failure("wrong session id " + messageKey)),
                                  Case.matchCase(() ->  key.equals(messageKey) && messageValue.equals("UPDATE"), () -> {
